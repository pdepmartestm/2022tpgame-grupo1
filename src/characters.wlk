...
object player inherits Character(position = game.at(0, 0), image = "player.png") {
...
object enemy inherits Character(position = game.at(6, 11), image = "enemy.png") {
	...
	method moveRandom() {
		...
		if (random == 1) {
			self.move(self.position().down(1), self.bloqAbajo())
		}
		if (random == 2) {
			self.move(self.position().left(1), self.bloqIzq())
		}
		if (random == 3) {
			self.move(self.position().right(1), self.bloqDer())
		}
	}
